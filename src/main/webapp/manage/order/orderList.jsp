...
<head>
    ...
    <script type="text/javascript">
        ...
        <body>
        <form action="order" namespace="/manage" method="post" theme="simple" <c:if test='${e.refundStatus=="WAIT_SELLER_CONFIRM_GOODS" || e.refundStatus=="WAIT_SELLER_AGREE"}'>hidden="hidden"</c:if>>
            <table class="table table-bordered">
                <tr>
                    ...
                    <td>
                        <select id="status" name="status" class="search-query input-small">
                            <option value=''></option>
                            <option value='init' <c:if test='${e.status=="init"}'>selected='selected'</c:if>>未审核
                            </option>
                            <option value='pass' <c:if test='${e.status=="pass"}'>selected='selected'</c:if>>已审核
                            </option>
                            <option value='send' <c:if test='${e.status=="send"}'>selected='selected'</c:if>>已发货
                            </option>
                            <option value='sign' <c:if test='${e.status=="sign"}'>selected='selected'</c:if>>已签收
                            </option>
                            <option value='cancel' <c:if test='${e.status=="cancel"}'>selected='selected'</c:if>>已取消
                            </option>
                            <option value='file' <c:if test='${e.status=="file"}'>selected='selected'</c:if>>已归档
                            </option>
                        </select>
                    </td>
                    <td>支付状态</td>
                ...
                <tr>
                    ...
                    <td colspan="21">
                        <select id="refundStatus" name="refundStatus" class="input-small">
                            <option value=''></option>
                            <option value='WAIT_SELLER_AGREE'
                                    <c:if test='${e.refundStatus=="WAIT_SELLER_AGREE"}'>selected='selected'</c:if>>
                                等待卖家同意退款
                            </option>
                            <option value='WAIT_BUYER_RETURN_GOODS'
                                    <c:if test='${e.refundStatus=="WAIT_BUYER_RETURN_GOODS"}'>selected='selected'</c:if>>
                                卖家同意退款，等待买家退货
                            </option>
                            <option value='WAIT_SELLER_CONFIRM_GOODS'
                                    <c:if test='${e.refundStatus=="WAIT_SELLER_CONFIRM_GOODS"}'>selected='selected'</c:if>>
                                买家已退货，等待卖家收到退货
                            </option>
                            <option value='REFUND_SUCCESS'
                                    <c:if test='${e.refundStatus=="REFUND_SUCCESS"}'>selected='selected'</c:if>>
                                退款成功，交易关闭
                            </option>
                        </select>
                    </td>
                ...
                <tr>
                    <td colspan="14">
                        <button method="${ctx}/manage/order/selectList.action" class="btn btn-primary"
